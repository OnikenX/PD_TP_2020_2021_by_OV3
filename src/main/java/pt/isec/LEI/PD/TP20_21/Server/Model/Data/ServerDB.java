...
//                System.out.println("utilizador 0:"+rs.getString("username")+", hash da password:"+rs.getString("hash"));
    ...
    public int addNewUser(String username, String name, String passwordtext) throws Exception {
        return addUser(-1, username, name, Password.getSaltedHash(passwordtext));
    }
    //users
    ...
    synchronized public int mensagemDM(String pessoaEnviaUsernome, String pessoaRecebeUsernome, String conteudo) throws SQLException {
        int canal_id;
        int pessoaEnvia = getUserId(pessoaEnviaUsernome);
        int pessoaRecebe = getUserId(pessoaRecebeUsernome);
        if (-1 == (canal_id = canalDmExists(pessoaEnvia, pessoaRecebe))) {
            ...
            getStatement().executeUpdate(
                    "insert into " +
                            "mensagens(authorId, canalId, isAFile, mensagem) " +
                            "values " +
                            "(" +pessoaEnvia + ", " + canal_id + ", " + 0 + ", " + conteudo + ")");
    ...
    public void verificaMudancas(ArrayList<Object> lista, String tabela) throws Exception {
        ...
        while (rs.next()) {
            switch (tabela) {
                ...
                case table_canaisGrupo:
                    id = rs.getInt(1);
                    rstemp = statement.executeQuery("select * from " + table_canais + ";");
                    rstemp.next();
...
//                        addMensagem(id, mensagem.getDataHoraEnvio(), mensagem.getAuthorId(), mensagem.getCanalId(), (mensagem.isAFile()?1:0), mensagem.getMensagem());
                ...
                case table_utilizadores:
                    ...
                    if (found) {
                        if (!(id == utilizadorServer.getId() &&
                            ...
                            statement.executeUpdate("UPDATE utilizadores SET " +
                                    ", username = " + utilizadorServer.getUsername() +
                                    ", nome = " + utilizadorServer.getNome() +
    ...
    public List<String> getLastMensagemDMs(int nmensagens) throws SQLException {
        List<String> listmensagens = new LinkedList<>();

        var qr = getConn().createStatement().executeQuery("select userdest.nome as destinatario, userauthor.nome as autor, mensagens.mensagem, mensagens.dataHoraEnvio as \"hora de envio\"\n" +
                "from mensagens\n" +
                "         inner join canaisDM on mensagens.canalId = canaisDM.id\n" +
                "         inner join utilizadores userauthor on userauthor.id = mensagens.authorId\n" +
                "         inner join utilizadores userdest on userdest.id = canaisDM.pessoaDest\n" +
                "order by mensagens.dataHoraEnvio desc\n" +
                "limit " + nmensagens + ";");
        while (qr.next()) {
            listmensagens.add("{"+qr.getString(1) + ", " + qr.getString(2) + ", " + qr.getString(3) + "}");
        }
        return listmensagens;
    }

//    private void addMensagem(int id, Timestamp dataHoraEnvio, int authorId, int canalId, int i, String mensagem){
//        getStatement().executeUpdate( "insert into " +
//                "mensagens(dataHoraEnvio, authorId, canalId, isAFile, mensagem) " +
//                "values " +
//                "("+timestamp+", "+pessoaEnvia+", "+canal_id+", "+
//                (isAFile? 1:0)+", "+conteudo+")");
