...
public class ClientServerConnection extends Thread {
    ...
    public void updateServer(Object enviar) {
        ...
        try {
            osTCP.write(Objects.requireNonNull(Utils.objectToBytes(enviar)));
        } catch (IOException e) {
            if (Utils.Consts.DEBUG) {
        ...
        try {
            while (true) {
                try {
                    ...
                    receivedObject = Utils.bytesToObject(receivedBytes2);
                    if(Utils.Consts.DEBUG)
    ...
    private InetAddress connectUdp(Conectar pedido) {
        try {
            ...
            var pedidoBytes = Utils.objectToBytes(pedido);
